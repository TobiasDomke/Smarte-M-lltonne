...
// #define WIFI_SSID "Palme 2,4 GHz"
// #define WIFI_PASSWORD "Wlandomkevirtrj"

#define WIFI_SSID "Imperator"
...
#define AUTHOR_EMAIL "smartemulltonne@gmail.com"
#define AUTHOR_PASSWORD "wzxh wzew zprz xfhn"
#define PIN_HIGH 13
...
int max_result = 5;
bool mail_recieved = false;

// Array to store the UID of messages in search result
int msg_uid[5];
int total_messages = 0;
void LED(){
  digitalWrite(PIN_HIGH, HIGH);  // Setze den Pin auf HIGH

    delay(10000);  // Warte 2 Sekunden

    digitalWrite(PIN_HIGH, LOW);  // Setze den Pin auf LOW
...
void printSelectedMailboxInfo(SelectedFolderInfo sFolder) {
  ...
  if (sFolder.msgCount()!=total_messages){
    total_messages = sFolder.msgCount();
    LED();
  }
...
void printMessageData() {
  ...
  if (std::string(msg.subject) == "Antwort auf <Mülleimer voll>") {
    LED();
  }
